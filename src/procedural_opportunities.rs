...
pub fn populate_generated_opportunities(state: &mut GameState) -> usize {
    ...
    if let Some((faction_id, target_id, kind, location_id)) = select_relationship(state) {
        ...
        if ensure_event(state, relationship_event(state, event_id, &kind, location_id)) {
            added += 1;
        }
        added
    } else {
...
mod tests {
    ...
    fn generated_world_gets_quest_and_event() {
        ...
        assert!(state.quests.iter().any(|q| q.content_id.starts_with(QUEST_PREFIX)));
        assert!(state.campaign_content.as_ref().unwrap().events.iter().any(|e| e.id.starts_with(EVENT_PREFIX)));
    }
    ...
    fn generated_quest_references_valid_entities() {
        ...
        let quest = state.quests.iter().find(|q| q.content_id.starts_with(QUEST_PREFIX)).expect("generated quest");
        assert!(state.world.location_by_id(quest.target_location_id).is_some());
        assert!(state.factions.iter().any(|f| f.id == quest.faction_id));
        assert!(state.npcs.iter().any(|n| n.id == quest.giver_npc_id));
    ...
    fn completed_generated_quest_evolves_world() {
        let mut state = prepared_state();
        populate_generated_opportunities(&mut state);
        let index = state.quests.iter().position(|q| q.content_id.starts_with(QUEST_PREFIX)).expect("generated quest");
        let location_id = state.quests[index].target_location_id;
        state.world.location_by_id_mut(location_id).unwrap().dangerous = true;
        ...
        assert!(state.quests.iter().any(|q| q.content_id.starts_with(EVOLUTION_QUEST_PREFIX)));
    ...
    fn generated_state_survives_serialization() {
        ...
        let index = state.quests.iter().position(|q| q.content_id.starts_with(QUEST_PREFIX)).expect("generated quest");
        state.quests[index].completed = true;
        state.world.location_by_id_mut(state.quests[index].target_location_id).unwrap().dangerous = true;
        evolve_generated_world(&mut state);
        let restored: GameState = serde_json::from_slice(&serde_json::to_vec(&state).unwrap()).unwrap();
        assert!(restored.quests.iter().any(|q| q.content_id.starts_with(EVOLUTION_QUEST_PREFIX)));
    }
}
