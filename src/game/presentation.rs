...
pub(crate) fn render_state(state: &GameState) {
    ...
    let condition_line = if character.conditions.is_empty() {
        ...
        Some(format!(
            "Condition: {}",
            character.conditions.iter().map(|c| format!("{} ({})", c.name, c.remaining)).collect::<Vec<_>>().join(", ")
        ))
    };
...
pub(crate) fn maybe_run_location_scene(state: &mut GameState) -> io::Result<()> {
    ...
    let content = state.campaign_content.clone().unwrap_or_else(crate::content::load_campaign_content);
