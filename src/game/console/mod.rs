...
fn refresh_completion(console: &mut ConsoleState, state: &GameState) {
    ...
    if console.selected_candidate >= console.candidates.len() {
        console.selected_candidate = 0;
    }
}
...
fn command_candidates() -> Vec<(String, String)> {
    [
        ...
        ("where", "content paths"),
        ("mods", "loaded mods and warnings"),
        ("content", "content counts"),
        ("locations", "list location IDs"),
        ("goto", "move by location ID"),
        ("teleport", "goto alias"),
        ("npcs", "list NPC IDs"),
        ("quests", "list quests"),
        ("quest", "quest commands"),
        ("factions", "list faction IDs"),
        ("faction", "faction commands"),
        ("inventory", "list items"),
        ("give", "clone inventory item"),
        ("remove", "remove item"),
        ("heal", "restore HP"),
        ("damage", "deal damage"),
        ("kill", "kill character"),
...
fn location_candidates(state: &GameState, prefix: &str) -> Vec<Candidate> {
    state
        ...
        .iter()
        .filter_map(|e| {
            let value = e.id.to_string();
            value.starts_with(prefix).then(|| Candidate {
                value,
                hint: e.name.clone(),
            })
        })
...
fn select_previous(console: &mut ConsoleState) {
    if !console.candidates.is_empty() {
        console.selected_candidate = console
            .selected_candidate
            .checked_sub(1)
...
fn accept_completion(console: &mut ConsoleState) {
    let Some(candidate) = console.candidates.get(console.selected_candidate).cloned() else {
        return;
    };
...
fn help(console: &mut ConsoleState) {
    ...
    console.output.push(
        "Tab: complete | Up/Down: choose completion | Enter: accept | Esc: cancel completion"
            .into(),
    );
}
...
fn status(state: &GameState, console: &mut ConsoleState) {
    ...
    console.output.extend([
        ...
        format!(
            "World: {} [{}] day={} time_points={}",
            state.world.name, state.world.id, state.world.day, state.world.time_points
        ),
...
fn where_content(console: &mut ConsoleState) {
    ...
    for root in roots {
        console.output.push(format!(
            "data: {} [{}]",
            root.display(),
            if root.is_dir() { "dir" } else { "missing" }
        ));
    }
}
...
fn content(state: &GameState, console: &mut ConsoleState) {
    if let Some(content) = &state.campaign_content {
        console.output.push(format!(
            "locations={} factions={} npcs={} quests={}",
        ...
        console.output.push(format!(
            "encounters={} events={} atmospheres={} item_visuals={}",
            content.encounters.len(),
            content.events.len(),
            content.atmospheres.len(),
            content.item_visuals.len()
        ));
    } else {
        console.output.push("campaign content: not loaded".into());
    }
}
fn locations(state: &GameState, console: &mut ConsoleState) {
    for e in &state.world.locations {
        console.output.push(format!("{} — {}", e.id, e.name));
    }
}

fn goto(state: &mut GameState, console: &mut ConsoleState, args: &[&str]) {
    let Some(id) = args.first().and_then(|v| v.parse::<EntityId>().ok()) else {
        console.output.push("usage: goto <location-id>".into());
        return;
    };
    let Some(name) = state.world.location_by_id(id).map(|l| l.name.clone()) else {
...
fn npcs(state: &GameState, console: &mut ConsoleState, args: &[&str]) {
    let filter = args.first().and_then(|v| v.parse::<EntityId>().ok());
    for npc in &state.npcs {
        if filter.is_some_and(|id| npc.location_id != id) {
            continue;
        }
        console
            .output
...
fn quest(state: &mut GameState, console: &mut ConsoleState, args: &[&str]) {
    match args {
        ...
        [action, value] if matches!(*action, "complete" | "reset") => {
            let Some(id) = value.parse::<EntityId>().ok() else {
                console.output.push("quest id must be numeric".into());
                return;
            };
            let Some(index) = state.quests.iter().position(|q| q.id == id) else {
            ...
            if *action == "complete" {
                ...
                if !state.world.completed_quest_ids.contains(&content_id) {
                ...
                state
                    .world
                    .completed_quest_ids
                    .retain(|id| id != &content_id);
                console.output.push(format!("reset {title}"));
            }
        }
        _ => console
            .output
            .push("usage: quest list | quest complete <id> | quest reset <id>".into()),
    }
}
...
fn factions(state: &GameState, console: &mut ConsoleState) {
    for f in &state.factions {
        console
            .output
            .push(format!("{} — {} ({:+})", f.id, f.name, f.reputation));
    }
}
fn faction(state: &mut GameState, console: &mut ConsoleState, args: &[&str]) {
    let ["set", id, value] = args else {
        console
            .output
            .push("usage: faction set <id> <reputation>".into());
        return;
    };
    let (Ok(id), Ok(value)) = (id.parse::<EntityId>(), value.parse::<i32>()) else {
        console.output.push("invalid id/reputation".into());
        return;
    };
    let Some(entry) = state.factions.iter_mut().find(|f| f.id == id) else {
...
fn give(state: &mut GameState, console: &mut ConsoleState, args: &[&str]) {
    let Some(id) = args.first().and_then(|v| v.parse::<EntityId>().ok()) else {
        console
            .output
            .push("usage: give <inventory-item-id> [count]".into());
        return;
    };
    let count = args
        .get(1)
    ...
    let Some(source) = state
        ...
        console.output.push(format!("unknown item id {id}"));
        return;
    };
    for _ in 0..count {
    ...
    console
        .output
        .push(format!("added {} x{}", source.name, count));
}
fn remove(state: &mut GameState, console: &mut ConsoleState, args: &[&str]) {
    let Some(id) = args.first().and_then(|v| v.parse::<EntityId>().ok()) else {
        console
            .output
            .push("usage: remove <inventory-item-id>".into());
        return;
    };
    let before = state.character.inventory.len();
    state.character.inventory.retain(|i| i.id != id);
    console
        .output
        .push(if before == state.character.inventory.len() {
...
fn heal(state: &mut GameState, console: &mut ConsoleState, args: &[&str]) {
    ...
    state.character.heal(value.max(0));
    console.output.push(format!(
        "HP {}/{}",
...
fn damage(state: &mut GameState, console: &mut ConsoleState, args: &[&str]) {
    let value = args
        ...
        .unwrap_or(1)
        .max(0);
    state.character.hp = (state.character.hp - value).max(0);
    if state.character.hp == 0 {
        state.character.alive = false;
    }
    console.output.push(format!(
        "HP {}/{} alive={}",
        state.character.hp, state.character.max_hp, state.character.alive
    ));
}
fn kill(state: &mut GameState, console: &mut ConsoleState) {
    state.character.hp = 0;
    state.character.alive = false;
    console.output.push("character killed".into());
}
fn revive(state: &mut GameState, console: &mut ConsoleState) {
    state.character.alive = true;
    state.character.hp = state.character.max_hp.max(1);
    console.output.push("character revived".into());
}
fn xp(state: &mut GameState, console: &mut ConsoleState, args: &[&str]) {
    let (mode, value) = match args {
        ["set", value] => ("set", value.parse::<u32>().ok()),
        ["add", value] | [value] => ("add", value.parse::<u32>().ok()),
        _ => ("add", None),
    };
    let Some(value) = value else {
        console.output.push("usage: xp set <n> | xp add <n>".into());
        return;
    };
    state.character.experience = if mode == "set" {
        value
    } else {
        state.character.experience.saturating_add(value)
    };
    console
        .output
        .push(format!("xp={}", state.character.experience));
}
fn level(state: &mut GameState, console: &mut ConsoleState, args: &[&str]) {
    let Some(value) = (match args {
        ["set", value] | [value] => value.parse::<u32>().ok(),
        _ => None,
    }) else {
        console.output.push("usage: level set <n>".into());
        return;
    };
...
fn attr(state: &mut GameState, console: &mut ConsoleState, args: &[&str]) {
    let args = if args.first().copied() == Some("set") {
        &args[1..]
    } else {
        args
    };
    let [name, value] = args else {
        console
            .output
            .push("usage: attr set <might|insight|endurance> <n>".into());
        return;
    };
...
fn condition(state: &mut GameState, console: &mut ConsoleState, args: &[&str]) {
    match args {
        ...
        ["add", name, remaining, penalty, bonus] => {
            let (Ok(remaining), Ok(penalty), Ok(bonus)) = (
                remaining.parse::<u32>(),
                penalty.parse::<i32>(),
                bonus.parse::<i32>(),
            ) else {
                console.output.push("invalid condition values".into());
                return;
            };
        ...
        _ => console.output.push(
            "usage: condition add <name> <remaining> <penalty> <bonus> | condition clear".into(),
        ),
    }
}
fn time_cmd(state: &mut GameState, console: &mut ConsoleState, args: &[&str]) {
    ...
    let Some(value) = value else {
        console
            .output
            .push("usage: time set <0..3> | time add <n>".into());
        return;
    };
    ...
    console
        .output
        .push(format!("time_points={}", state.world.time_points));
}
fn day_cmd(state: &mut GameState, console: &mut ConsoleState, args: &[&str]) {
    let Some(value) = (match args {
        ...
        console.output.push("usage: day set <n>".into());
        return;
    };
    state.world.day = value.max(1);
    console.output.push(format!("day={}", state.world.day));
}
fn history(state: &GameState, console: &mut ConsoleState, args: &[&str]) {
    let count = args
        .first()
        .and_then(|v| v.parse::<usize>().ok())
        .unwrap_or(10)
        .min(100);
    for e in state.world.history.iter().rev().take(count).rev() {
        console.output.push(format!("[{}] {}", e.turn, e.text));
    }
}
...
fn draw_console(
    ...
    terminal.draw(|frame| {
        ...
        let inner = block.inner(popup);
        frame.render_widget(block, popup);
        let candidate_height = if console.autocomplete {
            (console.candidates.len() as u16 + 2).min(inner.height.saturating_sub(7).max(3))
        } else {
            0
        };
        ...
        if candidate_height > 0 {
            let lines = console
                ...
                .enumerate()
                .map(|(index, c)| {
                    let marker = if index == console.selected_candidate {
                        '▶'
                    } else {
                        ' '
                    };
                    Line::from(format!("{marker} {:<12} {}", c.value, c.hint))
                })
                .collect::<Vec<_>>();
            frame.render_widget(
                Paragraph::new(lines)
        ...
        let x = chunks[2].x
            ...
            + console
                ...
                .chars()
                .count()
                .min(chunks[2].width.saturating_sub(3) as usize) as u16;
...
fn centered_rect(px: u16, py: u16, area: Rect) -> Rect {
    Rect {
        x: area.x + area.width.saturating_mul(100 - px) / 200,
        y: area.y + area.height.saturating_mul(100 - py) / 200,
        width: area.width.saturating_mul(px) / 100,
        height: area.height.saturating_mul(py) / 100,
    }
}
