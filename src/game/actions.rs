        ...
        match choice {
            ...
            1 => {
                ...
                for quest_index in quest_indices {
                    ...
                    handled = true;
                    if state.character.inventory.iter().any(|item| item.name == required_item_name) { complete_quest(state, quest_index); } else { println!("{} looks at you expectantly. You have not brought the required proof.", npc_name); }
                }
...
fn corpse_label(corpse: &Corpse) -> String {
    if corpse.former_name.is_empty() { "Unidentified remains".to_string() } else if corpse.scavenged { format!("{} the {} (searched)", corpse.former_name, corpse.former_title) } else { format!("{} the {}", corpse.former_name, corpse.former_title) }
}
...
pub(crate) fn add_or_refresh_condition(conditions: &mut Vec<Condition>, condition: Condition) {
    if let Some(existing) = conditions.iter_mut().find(|current| current.name == condition.name) { existing.remaining = existing.remaining.max(condition.remaining); existing.penalty = condition.penalty; existing.bonus = condition.bonus; } else { conditions.push(condition); }
}
...
pub(crate) fn gain_experience(state: &mut GameState, amount: u32) {
    ...
    loop {
        ...
        if let Ok(Some(choice)) = choose_from_list("Choose a new strength", &options, None) {
            match choice { 0 => state.character.attributes.might += 1, 1 => state.character.attributes.insight += 1, _ => state.character.attributes.endurance += 1 }
        }
    }
}
...
pub(crate) fn character_sheet(state: &GameState) {
    ...
    if state.factions.is_empty() { println!("Faction reputation: none"); } else { println!("Faction reputation:"); for faction in &state.factions { println!("  - {} {:+}", faction.name, faction.reputation); } }
    pause();
}
...
pub(crate) fn meditate_and_save(state: &mut GameState, save_path: &Path) -> std::io::Result<()> {
    ...
    let mut rested = Condition::new("Well-rested", 3, 0); rested.bonus = 1; add_or_refresh_condition(&mut state.character.conditions, rested);
    let character_name = state.character.display_name();
    ...
    narrate(&format!("You meditate until your breathing steadies. You look at the sky...\n{}\nYou recover {} HP and save the game.", crate::game::time::time_display(state.world.time_points, state.world.day), healing));
    Ok(())
}
...
pub(crate) fn review_quests(state: &GameState) {
    println!(); println!("Quest log for {}", state.character.display_name());
