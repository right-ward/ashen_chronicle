...
The CI workflow performs that native-library build and packaging automatically. APK packaging deliberately compresses the native `.so` library so the directly distributed APK is substantially smaller; this uses Gradle's `jniLibs.useLegacyPackaging` setting. Local release signing is optional; when the signing environment variables are absent, the Gradle release build remains unsigned.
