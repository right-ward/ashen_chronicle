...
The CI workflow performs that native-library build and packaging automatically. APK packaging deliberately compresses the native `.so` library so the directly distributed APK is substantially smaller; this uses Gradle's `packagingOptions.jniLibs.useLegacyPackaging` setting together with `android:extractNativeLibs="true"` in the application manifest so the packaging and runtime extraction behavior agree. Local release signing is optional; when the signing environment variables are absent, the Gradle release build remains unsigned.
